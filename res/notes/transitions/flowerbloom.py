...
def f(theta, k):
    r = k*(5+2*cos(5*theta))
    return r*cos(theta+(2*pi*k/50.0)),r*sin(theta+(2*pi*k/50.0))
...
with open("../../../src/fx/fx.hgr.flower.radial.data.a", "w") as f:
    for aval, bval in unique_vals:
        f.write("         !byte %s,%s\n" % (aval, bval))

ripple_vals = util.ripple(unique_vals)
with open("../../../src/fx/fx.hgr.flower.radial.ripple.data.a", "w") as f:
    for aval, bval in ripple_vals:
        f.write("         !byte %s,%s\n" % (aval, bval))

unique_vals.reverse()
with open("../../../src/fx/fx.hgr.flower.radial.in.data.a", "w") as f:
    for aval, bval in unique_vals:
        f.write("         !byte %s,%s\n" % (aval, bval))
