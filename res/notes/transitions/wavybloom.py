...
def f(theta, k):
    ...
    return r*cos(theta+(2*pi*k/30.0)),r*sin(theta+(2*pi*k/30.0))
...
with open("../../../src/fx/fx.hgr.wavy.iris.bloom.data.a", "w") as f:
    for aval, bval in unique_vals:
        f.write("         !byte %s,%s\n" % (aval, bval))

ripple_vals = util.ripple(unique_vals)
with open("../../../src/fx/fx.hgr.wavy.ripple.bloom.data.a", "w") as f:
    for aval, bval in ripple_vals:
        f.write("         !byte %s,%s\n" % (aval, bval))

unique_vals.reverse()
with open("../../../src/fx/fx.hgr.wavy.iris.bloom.in.data.a", "w") as f:
    for aval, bval in unique_vals:
        f.write("         !byte %s,%s\n" % (aval, bval))
